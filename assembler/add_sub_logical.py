...
def parse_args(op, op_args, variables_data):
    if ',' in op_args:
        if op_args == 'A,B':  # A,B
            return I[op]['variants']['ab']['lit'] + I[op]['variants']['ab']['signal'] + I[op]['operation_code']
        elif op_args == 'B,A': # B,A
            return I[op]['variants']['ba']['lit'] + I[op]['variants']['ba']['signal'] + I[op]['operation_code']
        elif op_args == 'A,(B)': # B,A
            return I[op]['variants']['adb']['lit'] + I[op]['variants']['adb']['signal'] + I[op]['operation_code']
        elif op_args == 'B,(B)': # B,A
            return I[op]['variants']['bdb']['lit'] + I[op]['variants']['bdb']['signal'] + I[op]['operation_code']
        args = op_args.split(',')
        if args[0] == 'A': 
            if '(' not in args[1]: 
                try: # A,lit
        ...
        if args[0] == 'B': 
            if '(' not in args[1]: # B,lit
                try: # B,lit
                    ...
                    memory_dir = args[1].strip('(').strip(')')
                    lit = to_binary(variables_data[memory_dir]['dir_memory'], 16)
                return lit + I[op]['variants']['ab']['signal'] + I[op]['operation_code']
            else: # B, (dir)
                memory_dir = args[1].strip('(').strip(')')
                lit = to_binary(memory_dir, 16) if memory_dir not in variables_data else to_binary(variables_data[memory_dir]['dir_memory'], 16)
                return lit + I[op]['variants']['bd']['signal'] + I[op]['operation_code']
    elif '(' in op_args:
